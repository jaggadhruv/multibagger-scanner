...
Computes per ticker:
  ...
  * Supertrend (ATR period=10, multiplier=2.5) — via src/supertrend.py
...
def _compute_indicators(ticker: str, hist: pd.DataFrame) -> dict[str, Any]:
    """Compute all indicators from a per-ticker OHLC DataFrame."""
    close = hist["Close"]
    ...
    # 52-week range (from actual bar data — more accurate than yfinance.info)
    ...
    # Supertrend via user's proven implementation
    ...
    if len(st_df) < 2:
        ...
        # Days in current trend: count trailing bars with same Direction
